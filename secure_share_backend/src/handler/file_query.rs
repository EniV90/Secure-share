...
use crate::{ db::FileRepository, dto::{RequestQueryDto, UserReceiveFileDto, UserRecieveListResponseDto, UserSendFileDto, UserSendFileListResponseDto}, error::HttpError, middleware::JWTAuthMiddleware, AppState};
...
pub async fn get_user_shared_files(
  ...
  let (shared_files, total_count) = app_state.file_repository
...
pub async fn get_receive_shared_files(
  ...
  let (receive_files, total_count) = app_state.file_repository
  .get_receive_files(user_id.clone(), page as u32, limit)
  .await
  .map_err(|e|HttpError::server_error(e.to_string()))?;

  let filter_receive_files = UserReceiveFileDto::filter_recieve_user_files(&receive_files);

  let response = UserRecieveListResponseDto {
    status: "success".to_string(),
