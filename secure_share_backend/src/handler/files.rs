...
use crate::{db::{FileRepository, SharedLinkRepository, UserRepository}, dto::{FileUploadDto, Response as ResponseDto, RetrieveFileDto}, error::HttpError, middleware::JWTAuthMiddleware, utils::{decrypt::decrypt_file, encrypt::encrypt_file, password}, AppState};
...
pub async fn upload_file(
  ...
  while let Some(field) = multipart.next_field().await.unwrap() {
    ...
    match name.as_str() {
      "fileUpload" => {
        file_name = field.name().unwrap_or("unknown_file").to_string();
  ...
  let recipient_result = app_state.user_repository
...
app_state.file_repository
.save_encrypted_file(user_id.clone(), file_name, file_size, recipient_user_id, hash_password, expiration_date, encrypted_aes_key, encrypted_data, iv)
.await
.map_err(|e|HttpError::server_error(e.to_string()))?;
...
pub async fn retrieve_file(
  ...
  let shared_result = app_state.shared_link_repository
  ...
  let file_id = match shared_data.file_id {
    ...
    None => {
      //Handle case when file id is None
      return Err(HttpError::bad_request("File ID is missing".to_string()));
    }
  };

  let  file_result = app_state.file_repository
  .get_file(file_id.clone())
  .await
  .map_err(|e| HttpError::server_error(e.to_string()))?;

  let file_data = file_result.ok_or_else( || {
    HttpError::bad_request("The requested file either does not exit or has expired.".to_string())
  })?;
  ...
  let decrypted_file = decrypt_file(file_data.encrypted_aes_key, file_data.encrypted_file, file_data.iv, &private_key_pem)
  .await?;
