...
use crate::{db::UserRepository, dto::{EmailListResponse, FilterEmailDto, FilterUserDto, NameUpdateDto, Response, SearchQueryByEmailDto, UserData, UserPasswordUpdateDto, UserResponseDto}, error::{ErrorMessage, HttpError}, middleware::JWTAuthMiddleware, utils::password, AppState};
...
pub async fn search_by_email(
  ...
  let users = app_state.user_repository
  ...
  let response_data = EmailListResponse {
    status: "success".to_string(),
